...
exports.options = function (options) {
	...
	if (typeof options === 'object') {
		...
		while (i--) {
			k = keys[i];
			// if non-string variable
			// if (typeof options[k] === 'object') {
			// 	let newOptions = options[k];
			// 	if (this._options[k]) {
			// 		// override the orginial structure
			// 		newOptions = Object.assign({}, this._options[k], options[k]);
			// 	}
			// 	console.log('Set new options: ', k, newOptions);
			// 	this.set(k, newOptions);
			// } else {
