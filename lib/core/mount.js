...
function mount(mountPath, parentApp, events) {
  debug('mounting');
	...
	if (!this.get('mongo')) {
		var dbUrl = this.get('db url');
		if(!dbUrl){
			var dbName = this.get('db name') || utils.slug(this.get('name'));
			dbUrl = process.env.MONGO_URI || process.env.MONGO_URL || process.env.MONGODB_URL || process.env.MONGOLAB_URI || process.env.MONGOLAB_URL || (process.env.OPENSHIFT_MONGODB_DB_URL || 'mongodb://localhost/') + dbName;
		}
