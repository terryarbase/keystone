...
function List(key, options) {
	...
	if(options && options.mongoose){
		this.schema = new options.mongoose.Schema({}, this.options.schema);
	}
	else{
...
List.prototype.register = function() {
	...
	if (this.get('inherits')) {
		...
		if(this.options && this.options.mongoose && this.options.dbConnection){
			this.model = this.options.dbConnection.model(this.key, this.schema);
		}
		else{
